<?php 
	...
	$_SESSION['site']="quarterFinal";
include("connection.php");
include("main_back.php");

	$q="select *,count(*) as count from `match` where round_number=3 group by match_id order by match_time";
	$qu=mysql_query($q);
	?>
	<br>
	<?php
	...
	<?php } 
		$q="select count(*) as count from `match` where round_number=3";
		$qu=mysql_query($q);
		$donne=mysql_fetch_array($qu);
		if($donne['count']==0)
		{
		?>
			<h1 align="center"style="color:white;background-color:black">NO QUARTER FINAL YET</h1>
<?php	}?>
			<br><br>